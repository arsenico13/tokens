...
class Macro(object):
    ...
    def __str__(self):
        return "%s<%s,grp=%s>" % (self.__class__.__name__, self.label, self.group)
    ...
    @property
    def command(self):
        return jinja2.Template(self._command).render(macro=self)
    ...
    def color(self):
        return (
            {"Health": "green", "Action": "black"}.get(self.group, "black")
            if self._bcolor is None
            else self._bcolor
        )
    ...
    @property
    def fontColor(self):
        return self._fcolor or "white"

    @fontColor.setter
    def fontColor(self, v):
        self._fcolor = v

    @property
    def colors(self):
        return self.fontColor, self.color  # tuple (font, bg)

    @colors.setter
    def colors(self, fbg):
        self.fontColor, self.color = fbg
...
[macro("Description@Lib:Addon5e"):data]"""
            % (action["name"], action["desc"].encode("ascii", "xmlcharrefreplace")),
        )

    @property
    def group(self):
        return self._group or "Misc"
...
class ActionMacro(DescrMacro):
	...
	"ButtonColor","green",
	"FontColor","white")]
[macro("NPCAttack@Lib:Addon5e"):jsonWeaponData]"""
                % (
                    label,
                    self.damage_dice,
                    self.damage_bonus,
                    self.damage_type,
                    self.attack_bonus,
                    action["desc"].encode("ascii", "xmlcharrefreplace"),
                    token.name,
                ),
            )
        else:
    ...
    def hit_dd_db_type(self):
        match = re.search(
            r"\+(\d+) to hit,.*\((\d+d\d+) \+ (\d+)\) (\w+) damage", self.desc
        )
    ...
    @property
    def group(self):
        return self._group or "Action"


class LairMacro(ActionMacro):
    @property
    def color(self):
        return self._bcolor or "orange"

    @property
    def fontColor(self):
        return self._fcolor or "black"

    @property
    def group(self):
        return self._group or "Lair (on init 20)"


class RegionalEffectMacro(ActionMacro):
    @property
    def color(self):
        return self._bcolor or "blue"
    ...
    @property
    def group(self):
        return self._group or "Regional Effects"
...
class LegendaryMacro(ActionMacro):
    ...
    @property
    def group(self):
        return self._group or "Legendary"


class SpecialMacro(DescrMacro):
    @property
    def group(self):
        return self._group or "special"

    @property
    def color(self):
        return self._bcolor or "maroon"
...
class SpellMacro(Macro):
    def __init__(self, token, spell):
        ...
        Macro.__init__(
            ...
            token,
            spell.js,
            spell.name + (suffix if suffix != "()" else ""),
            jinja2.Template(spellTemplate).render(spell=spell, token=token),
        )
