...
class Dnd5ApiObject(object):
    ...
    def load(cls, build_dir):
        ...
        if os.path.exists(fp):
            log.warning(
                "Found serialized %ss, delete %s to refresh the items from %s"
                % (cls.__name__, fp, ubase)
            )
    ...
    def __getattr__(self, attr):
        if attr.lower() == "charisma" and self.js["name"] == "Aboleth":
            return 18  # 5e api bug
        ...
        if v is None:
            raise RuntimeError("Cannot find the attribute %s" % attr)
        return v
...
class Spell(Dnd5ApiObject):
    ...
    @property
    def html_desc(self):
        return " ".join(self.js["desc"]).replace("'", "&#39;")

    @property
    def desc(self):
        return " ".join(self.js["desc"])

    @property
    def classes(self):
        return [item["name"] for item in self.js["classes"]]
    ...
    @property
    def school(self):
        return self.js["school"]["name"]

    @property
    def concentration(self):
        return int(self.js["concentration"] == "yes")
    ...
    @property
    def attack(self):
        return int("spell attack" in self.desc)
    ...
    @property
    def components(self):
        return ", ".join(self.js["components"])

    @property
    def ritual(self):
        return int(self.js["ritual"] == "yes")
...
class Token(Dnd5ApiObject):
    ...
    def __repr__(self):
        return "Token<name=%s,attr=%s,hp=%s(%s),ac=%s,CR%s,img=%s>" % (
            self.name,
            [
                self.strength,
                self.dexterity,
    ...
    def assets(self):
        if self._assets is None:
            ...
            ratio = lambda name: difflib.SequenceMatcher(
                None, name.lower(), self.name.lower()
            ).ratio()
            # morph "/abc/def/anyfile.png" into "anyfile"
            short_name = lambda full_path: os.path.splitext(
            ...
            if self.pngs:
                ...
                bfpath, bratio = max(
                    itertools.chain(ratios, [("", 0)]), key=lambda i: i[1]
                )
            ...
            if bratio > 0.8 and (
                self.js.get("ref", "") != "Tome of Beast" or not args.delivery
            ):
    ...
    @property
    def bcon(self):
        return self.abonus("constitution")

    @property
    def bdex(self):
        return self.abonus("dexterity")

    @property
    def bwis(self):
        return self.abonus("wisdom")
    ...
    def sc(self):
        return next(
            (spe for spe in self.specials if spe["name"] == "Spellcasting"), None
        )
    ...
    def scAttributes(self):  # spellcasting attribute
        ...
        attr = next(
            (attr for attr in ["intelligence", "charisma", "wisdom"] if attr in desc),
            None,
        )
        ...
        if attack is None and attr:
            attack = self.abonus(attr)
        return (attr, dc, attack) if desc and attr and dc and attack else None

    @property
    def actions(self):
        return self.js.get("actions", [])

    @property
    def specials(self):
        return self.js.get("special_abilities", [])

    @property
    def legends(self):
        return self.js.get("legendary_actions", [])

    @property
    def lair_actions(self):
        return self.js.get("lair_actions", [])

    @property
    def regional_effects(self):
        return self.js.get("regional_effects", [])

    @property
    def passive_perception(self):
        return 10 + self.js.get("perception", self.bwis)

    @property
    def vulnerabilities(self):
        return self.js.get("damage_vulnerabilities", "")
    ...
    def skills(self):
        ...
        if skills == "":
            skills = ", ".join(
                [
                    "%s +%d" % (sk, self.js[sk])
                    for sk in all_skills().keys()
                    if self.js.get(sk, None)
                ]
            )
        return skills
    ...
    def saves(self):
        ...
        if saves == "":
            saves = ", ".join(
                [
                    "%s %+d" % (attr[:3], self.js["%s_save" % attr.lower()])
                    for attr in self.attributes
                    if "%s_save" % attr.lower() in self.js
                ]
            )
        return saves
    ...
    @property
    def immunities(self):
        return self.js.get("damage_immunities", "")

    @property
    def resistances(self):
        return self.js.get("damage_resistances", "")
    ...
    def macros(self):
        ...
        actions = (
            macros.ActionMacro(self, action)
            for action in self.actions
            if action["name"]
        )
        lairs = (
            macros.LairMacro(self, action)
            for action in self.lair_actions
            if action["name"]
        )
        reg = (
            macros.RegionalEffectMacro(self, action)
            for action in self.regional_effects
            if action["name"]
        )
        legends = (
            macros.LegendaryMacro(self, leg) for leg in self.legends if leg["name"]
        )
        ...
        if attributes:
            ...
            spellCast = (
                macros.SpellCastingMacro(self, spe, groupName)
                for spe in self.specials
                if spe["name"].lower() == "spellcasting"
            )
        specials = (
            macros.SpecialMacro(self, spe)
            for spe in self.specials
            if spe["name"] and spe["name"].lower() != "spellcasting"
        )
        ...
        commons = [
            ...
            macros.Macro(
                ...
                None,
                "Init",
                '[macro("Init@Lib:Addon5e"):0]',
                **{
                    "group": "Rolls",
                    "colors": ("white", "green"),
                    "tooltip": "Roll and add to the init panel",
                }
            ),
            macros.Macro(
                self,
                None,
                "SaveMe",
                '[macro("SaveMe@Lib:Addon5e"):0]',
                **{
                    "group": "Rolls",
                    "colors": ("white", "green"),
                    "tooltip": "Roll Saving Throws",
                }
            ),
            macros.Macro(
                self,
                None,
                "CheckMe",
                '[macro("CheckMe@Lib:Addon5e"):0]',
                **{
                    "group": "Rolls",
                    "colors": ("white", "green"),
                    "tooltip": "Roll Skill Checks",
                }
            ),
        ]
        if not args.delivery:
            commons.append(
                macros.Macro(
                    ...
                    "Debug",
                    "[r: a5e.debug()]",
                    **{"group": "zDebug", "colors": ("white", "black")}
                )
            )
        self._macros = list(
            itertools.chain(
                actions, spellCast, specials, legends, lairs, reg, commons, spells
            )
        )
        return self._macros
    ...
    def spells(self):
        spells = []
        for ability in (
            a for a in self.specials if "spellcasting" in a["name"].lower()
        ):
            spells.extend(
                [s for s in Spell.spellDB if s.name.lower() in ability["desc"]]
            )
        return spells
    ...
    def props(self):
        return (
            ...
            for name, value in [
                ...
                (
                    "Initiative",
                    '[h,macro("getNPCInitBonus@Lib:Addon5e"):0][r: macro.return]',
                ),
            ...
            + [
                ("b%s" % a[:3].lower(), '{floor((getProperty("%s")-10)/2)}' % a)
    ...
    def pngs(self):
        if self.pngFiles is self.sentinel:
            Token.pngFiles = list(
                itertools.chain(
                    *(
                        glob.glob(os.path.join(os.path.expanduser(imglib), "*.png"))
    ...
    @property
    def states(self):
        return [s for s in [State("Concentrating", "false")]]
        ...
        with zipfile.ZipFile(filename, "w", zipfile.ZIP_STORED) as zipme:
            ...
            for name, asset in self.assets.iteritems():
                zipme.writestr(
                    "assets/%s" % asset.md5,
                    jenv()
                    .get_template("md5.template")
                    .render(name=name, extension="png", md5=asset.md5)
                    .encode("utf-8"),
                )
...
class POI(LibToken):
    def __init__(self, name):
        ...
        self._assets = {}
        self._assets["null"] = Img(
            imglib + "/../GUI_Icons_png/transparent/location_t.png"
        )
        self._assets["chest"] = Img(
            imglib + "/../GUI_Icons_png/transparent/chest_t.png"
        )
        self._assets["gold"] = Img(imglib + "/../GUI_Icons_png/transparent/gold_t.png")
        self._assets["quest_c"] = Img(
            imglib + "/../GUI_Icons_png/transparent/quest_complete_t.png"
        )
        self._assets["quest"] = Img(
            imglib + "/../GUI_Icons_png/transparent/quest_t.png"
        )
        self._assets["magnifier"] = Img(
            imglib + "/../GUI_Icons_png/transparent/magnifier_t.png"
        )
        for num in range(21):
            fpath = os.path.join(imglib, "Room Numbers/Numbers/Marker %02d.png" % num)
            if os.path.exists(fpath):
                self._assets["rn_%02d" % num] = Img(fpath)
    ...
    def macros(self):
        if not self._macros:
            for name, asset in self.assets.iteritems():
                ...
                label = '<!--%s--><img height=40 width=40 src="asset://%s"></img>' % (
                    name,
                    asset.md5,
                )
                self._macros.append(
                    macros.Macro(
                        ...
                        "",
                        label,
                        """ [h: setTokenImage("asset://%s")] """ % asset.md5,
                        group="icons" if (not name.startswith("rn_")) else "IDs",
                        colors=("black", "white"),
                    )
                )
            self._macros.append(
        ...
        return (
            ...
            for name, value in [
                (
                    "images",
                    json.dumps(
                        {name: asset.md5 for (name, asset) in self.assets.iteritems()}
                    ),
                )
            ]
        )
...
def loadFromRst(fdata):
    ...
    header, content = re.split(
        "\n-+\n", rst
    )  # a line of '------' should be unique in the rst
    # sections = re.split('\n~+\n', content)
    ...
    size, _type, subtype, align = re.search(
        r"(\w+) (\w+) ?(\(.*?\))?, (.*)", stats
    ).groups()
    ...
    st, dex, con, intel, wis, cha = [
        int(e) for e in re.search(r"\| (\d+) \(.?\d+\)\s+" * 6, stats).groups()
    ]
    ...
    def getme(what, pattern, default=None):
        if "**%s**" % what not in stats and "**%s:**" % what not in stats:
            if default is None:
                raise RuntimeError("%s not found" % what)
            return default
        return re.search(
            "\*\*%s:?\*\* " % what + pattern, stats, re.MULTILINE | re.DOTALL
        ).group(1)
    ...
    for iname, section_name in [
        ...
        items[iname] = [
            (field, remove_ref(value).replace("-", " "))
            for field, value in items[iname]
        ]
    items["specials"] = [
        (field, remove_ref(value).replace("-", " ")) for field, value in specials
    ]
    ...
    for field, value in items["specials"]:
        if (
            "day each" in field.lower()
            or "at will" in field.lower()
            or "innate" in field.lower()
        ):
            spellCasting += value + " "
    ...
    return {
        ...
        "special_abilities": [
            {"name": field, "desc": value} for field, value in items["specials"]
        ],
        "actions": [
            {"name": field, "desc": value} for field, value in items["actions"]
        ],
        "reactions": [
            {"name": field, "desc": value} for field, value in items["reactions"]
        ],
        "legendary_actions": [
            {"name": field, "desc": value} for field, value in items["lactions"]
        ],
    }
...
def main():
    ...
    sources += [
        os.path.join(dp, f)
        for dp, dn, filenames in os.walk(tob)
        for f in filenames
        if os.path.splitext(f)[1] == ".rst" and "index" not in f
    ]
    for f in sources:
    ...
    fh = logging.FileHandler(
        os.path.join("build", "tokens.log"), mode="w"
...
"""
    ...
    addon.add(
        macros.Macro(
            ...
            "",
            "debug",
            """[h: props = getPropertyNames()] [foreach(name, props, "<br>"), code: { [name]: [getProperty(name)]: [getRawProperty(name)]}] """,
            **params
        )
    )
    addon.add(macros.Macro(addon, "", "output", fromFile("output.mtmacro"), **params))
    addon.add(
        macros.Macro(addon, "", "rollDice", fromFile("rollDice.mtmacro"), **params)
    )
    addon.add(
        macros.Macro(
            addon,
...
""",
    ...
    addon.add(
        macros.Macro(
            addon, "", "Description", fromFile("description.mtmacro"), **params
        )
    )
    addon.add(
        macros.Macro(addon, "", "CastSpell", fromFile("castSpell.mtmacro"), **params)
    )
    addon.add(
        macros.Macro(addon, "", "NPCAttack", fromFile("npcAttack.mtmacro"), **params)
    )
...
[h: macro.return = jskills]""",
    ...
    addon.add(
        macros.Macro(
            ...
            "",
            "getNPCSaves",
            r"""[h: id = strfind(getProperty("saves"), "((\\w+) \\+(\\d+))")]
[h: jsaves= "{}"]
[h: find = getFindCount(id)]
...
[h: macro.return = jsaves]""",
    ...
    addon.add(
        macros.Macro(
            ...
            "",
            "ControlPanel",
            """[dialog("A5e Panel", "width=215; height=700; temporary=0; input=1"): {[r,macro("cpanel@this"):0]}]""",
            **params
        )
    )
    params = {"group": "Format"}
...
<td width='[r:arg(0)]%'>
    ...
    sTokens = (
        []
    )  # used for further serialization, because tokens is a generator and will be consumed
    cnt = 0
    deliveryFilename = "build/dnd5eTokens.zip"
    zfile = (
        zipfile.ZipFile(deliveryFilename, "w", zipfile.ZIP_STORED)
        if args.delivery
        else None
    )
    ...
    for token in itertools.islice(tokens, args.max_token):
        ...
        if "dft.png" in token.img.name:
            log.warning(str(token))
        cnt += 1
